...
String gpsTime() {
    ...
    return hour + ":" + minute; */
    if (gps.time.isValid()) {
    if (gps.time.hour() == 0 && gps.time.minute() == 0) {
        // Handle the case when the time is 00:00 (midnight)
    }

    String hour = String(gps.time.hour());
    String minute = String(gps.time.minute());
} else {
    // Handle the case when the GPS time is not valid
}
...
void loop() {
    ...
    if (digitalRead(MODE_BUTTON) == LOW && currentTime - lastPageSwitchTime >= 300) {
    ...
    if (sosCallInProgress) {
        ...
        SerialUSB.println("Making call to 9944546840\nCall duration: " + formatElapsedTime(sosCallElapsedTime));
